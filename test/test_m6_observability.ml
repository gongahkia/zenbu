...
let document ?(selections = []) id contents =
  Document_id.of_string id |> must |> fun id ->
  Document.create ~id ~contents ~initial_selections:selections () |> must

let key text = Input_event.logical_text text |> must |> Input_event.key_press

let contains text fragment =
  String.contains text fragment.[0]
  && String.length text >= String.length fragment
  &&
  let rec loop index =
...
let test_trace_bounds_and_disabled () =
  ...
  Trace.emit_lazy disabled (fun () ->
      Trace_event.Input_received { execution_id = 1; input = "x" });
    ...
    (fun execution_id ->
      Trace.emit_lazy trace (fun () ->
          Trace_event.Input_received
            { execution_id; input = string_of_int execution_id }))
    [ 1; 2; 3 ];
...
let test_provenance_and_why () =
  let runtime =
    Vim_runtime.create ~commands:(registry ()) ~trace:(trace ())
      ~profiler:(profiler ())
  ...
  expect
    (Vim_runtime.change_ids pending = [])
    "pending delete committed a transaction";
  let pending_why =
    Inspector.why
      (Vim_runtime.trace runtime)
      ~execution_id:(Vim_runtime.execution_id pending)
    |> Option.get
  in
  let pending_text = Inspector.format_why pending_why |> String.concat "\n" in
  expect
    (contains pending_text "normal -> operator-pending")
    "pending explanation omitted generic transition";
  let runtime, changed = Vim_runtime.handle_input runtime (key "w") |> must in
  expect
    (List.length (Vim_runtime.change_ids changed) = 1)
    "dw did not record one change";
  let history_change =
    History.current_change (Vim_runtime.history runtime) |> Option.get
  in
  let provenance =
    History.transaction history_change
    |> Transaction.metadata_of |> Transaction.provenance |> Option.get
  in
  let entries = Provenance.entries provenance in
  expect
    (List.exists
       (function
         | Provenance.Model { id = "zenbu.vim-style"; _ } -> true | _ -> false)
       entries)
    "transaction provenance omitted the model";
  expect
    (List.exists
       (function
         | Provenance.Command { id = "editor.apply"; _ } -> true | _ -> false)
       entries)
    "transaction provenance omitted command origin";
  expect
    (List.exists
       (function Provenance.Selector "next-word" -> true | _ -> false)
       entries)
    "transaction provenance omitted selector identity";
  expect
    (List.exists
       (function Provenance.Transformation "delete" -> true | _ -> false)
       entries)
...
let test_input_rules_and_selection_atomicity () =
  let runtime =
    Vim_runtime.create ~commands:(registry ())
      ~document:(document "rules" "alpha") ()
    |> must
  in
  ...
  expect
    (List.exists
       (fun rule -> Input_rule.pattern rule = Input_rule.Exact "w")
       rules)
  ...
  let runtime, deleted =
    Selection_runtime.handle_input runtime (key "d") |> must
  in
  let change =
    History.current_change (Selection_runtime.history runtime)
    |> Option.get |> Inspector.change
  in
  expect
    (Inspector.change_edit_count change = 3)
    "multi-selection delete did not preserve three atomic edits";
  let why =
    Inspector.why
      (Selection_runtime.trace runtime)
      ~execution_id:(Selection_runtime.execution_id deleted)
    |> Option.get |> Inspector.format_why |> String.concat "\n"
  in
  expect
    (contains why "(3 selections)")
...
let test_structural_provenance_and_syntax_status () =
  ...
  let runtime =
    Structural_runtime.create ~commands:(registry ()) ~syntax_service:service
      ~trace:(trace ())
      ~document:(document "structural-observe" "let alpha = 1\n")
      ()
    |> must
  in
  let runtime, focused =
    Structural_runtime.handle_input runtime (key "f") |> must
  in
  let change =
    History.current_change (Structural_runtime.history runtime) |> Option.get
  in
  let provenance =
  ...
  expect
    (List.exists
       (function Provenance.Selector "syntax.focus" -> true | _ -> false)
       provenance)
    "structural focus omitted syntax selector provenance";
  let why =
    Inspector.why
      (Structural_runtime.trace runtime)
  ...
  expect
    (Inspector.syntax_service_cached_version status = Some 1)
    "syntax service status omitted cached document version";
  expect
    (Option.is_some (Inspector.syntax_service_last_strategy status))
    "syntax service status omitted strategy"

let test_history_view_and_descriptor_registry () =
  let initial = History.create (document "history-observe" "abcdef") in
  let initial_view = Inspector.history ~saved_version:0 initial in
  expect
    (initial_view |> Inspector.history_nodes
    |> List.exists (fun node ->
        Inspector.history_node_id node = 0 && Inspector.history_node_saved node)
    )
    "history view did not mark the saved root";
  let history = initial in
  let history =
    History.apply_intent ~source:Transaction.Test history
      (Intent.Insert_text "x")
    |> must
  in
  ...
  expect
    (List.length (Inspector.history_nodes view) = 3)
    "history view omitted a branch node";
  expect
    (Option.is_some
       (Inspector.find_change view (History.change_id lineage_change)))
    "history view could not find retained branch change";
  let formatted = Inspector.format_history view |> String.concat "\n" in
  expect (contains formatted "parent=") "history output omitted parent links";
  expect (contains formatted "action=") "history output omitted change actions";
  let descriptor = List.hd (Selector.descriptors ()) in
  let registry =
    Semantic_registry.register Semantic_registry.empty descriptor |> must
  in
  expect
...
let test_profiler_is_bounded_and_nonsemantic () =
  ...
  expect
    (Profiler.aggregates disabled = [])
    "disabled profiler retained samples";
  ...
  expect
    (Profiler.aggregate_count aggregate = 2)
    "profile capacity did not evict oldest samples";
  expect
    (Profiler.aggregate_max_seconds aggregate >= 0.)
  ...
  let with_profile =
    Vim_runtime.create ~commands:(registry ()) ~profiler
      ~document:(document "profiled" "alpha")
      ()
    |> must
    |> fun runtime ->
    Vim_runtime.handle_input runtime (key "i") |> must |> fst |> fun runtime ->
    Vim_runtime.handle_input runtime (Input_event.text_input "X" |> must)
    |> must |> fst
  in
  let without_profile =
    Vim_runtime.create ~commands:(registry ())
      ~document:(document "plain" "alpha") ()
    |> must
    |> fun runtime ->
    Vim_runtime.handle_input runtime (key "i") |> must |> fst |> fun runtime ->
    Vim_runtime.handle_input runtime (Input_event.text_input "X" |> must)
...
let test_failed_and_repeated_execution_context () =
  ...
  in
  expect
    (Result.is_error (Vim_runtime.handle_input runtime (key "p")))
    "empty paste unexpectedly succeeded";
  let failure =
    Inspector.latest_why trace |> Option.get |> Inspector.format_why
    |> String.concat "\n"
  in
  expect
    (contains failure "clipboard slot is empty")
  ...
  in
  let provenance =
    History.transaction repeated_change
    |> Transaction.metadata_of |> Transaction.provenance |> Option.get
    |> Provenance.entries
  in
  expect
    (List.exists
       (function
         | Provenance.Repeat "apply:next-word:delete" -> true | _ -> false)
       provenance)
  ...
  in
  expect
    (contains why "repeat apply:next-word:delete")
...
let () =
  [
    ...
    ( "input rules and selection atomicity",
      test_input_rules_and_selection_atomicity );
    ( "structural provenance and syntax status",
      test_structural_provenance_and_syntax_status );
    ( "history view and descriptor registry",
      test_history_view_and_descriptor_registry );
    ( "profiler bounded and nonsemantic",
      test_profiler_is_bounded_and_nonsemantic );
    ( "failed and repeated execution context",
      test_failed_and_repeated_execution_context );
  ]
