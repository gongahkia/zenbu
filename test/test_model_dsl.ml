...
let diagnostic_contains value needle =
  let rendered =
    diagnostics value |> List.map Dsl.Diagnostic.format |> String.concat "\n"
  in
  expect
    ...
    let rec contains index =
      ...
      else if String.sub rendered index (String.length needle) = needle then
        true
...
let context () =
  let document =
    Document.create
      ~id:(Document_id.of_string "dsl-test" |> must)
      ~contents:"alpha beta" ()
    |> must
  in
...
let test_diagnostics () =
  ...
  diagnostic_contains
    "zenbu-model 2\n\
     ...
     keys } } }"
    "unsupported zenbu-model version"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1model \"x\" { title \"X\" initial a state a { status { label \
     \"A\" input keys } } }"
    "must start with `zenbu-model 1`"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     keys } } state a { status { label \"A\" input keys } } }"
    "duplicate state `a`"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" state a { status { label \"A\" input keys } } }"
    "requires exactly one `initial`"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial missing state a { status { label \"A\" \
     input keys } } }"
  ...
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     keys } on \"g\" -> a on \"g g\" -> a } }"
    "use an explicit intermediate state"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     text } on \"<text>\" as first -> a on \"<text>\" as second -> a } }"
    "duplicate or overlapping"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     text } on \"a <text>\" -> a } }"
    "must be the entire input sequence"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     keys } on \"<text>\" as text -> a { insert $text } } }"
    "only valid in a state with `input text`"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     text } on \"<text>\" as text -> a { apply selector \"missing\" transform \
     \"delete\" } } }"
  ...
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     text } on \"<text>\" as text -> a { apply selector \"current-word\" \
     transform \"missing\" } } }"
    "unknown transformation `missing`"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
     text } on \"<text>\" -> a { insert $text } } }"
    "undefined text capture `$text`"
  |> ignore;
  diagnostic_contains
    "zenbu-model 1\n\
     model \"x\" { title \"X\" initial a state a { status { label \"A\" input \
  ...
  expect
    (Dsl.Diagnostic.line malformed = 2)
    "parser diagnostic lost source line";
  expect
    (Dsl.Diagnostic.column malformed > 0)
    "parser diagnostic lost source column";
  let crlf = diagnostics "zenbu-model 1\r\nmodel \"é\" {\r\n" |> List.hd in
  expect
    (Dsl.Diagnostic.line crlf = 3)
    "CRLF source diagnostic has the wrong line";
  expect
  ...
  let carriage_return_source =
    "zenbu-model 1\r# a CR-delimited comment\rmodel \"x\" { title \"X\" \
     initial a state a { status { label \"A\" input keys } } }"
  in
...
let test_runtime () =
  ...
  expect_string ~expected:"insert"
    ~actual:(Model_status.id (Dsl.Runtime.status state));
  ...
  let mismatched, effects =
    Dsl.Runtime.handle_input pending (key "x") context
  in
  ...
  let left, left_effects =
    Dsl.Runtime.handle_input (Dsl.Runtime.initialize compiled) (key "d") context
  in
  let right, right_effects =
    Dsl.Runtime.handle_input (Dsl.Runtime.initialize compiled) (key "d") context
  in
  expect
    (left_effects = right_effects)
    "identical runs produced different effects";
  expect
...
let test_input_rules_and_adapter () =
  ...
  let insert_state, _ =
    Dsl.Runtime.handle_input
      (Dsl.Runtime.initialize compiled)
      (key "i") (context ())
  in
  expect
    (List.exists
  ...
  let document =
    Document.create
      ~id:(Document_id.of_string "dsl-adapter" |> must)
      ~contents:"alpha" ()
    |> must
  in
  let runtime =
...
let test_branching_and_deep_prefixes () =
  ...
  let after_word, effects =
    Dsl.Runtime.handle_input after_end (key "w") context
  in
  ...
  let deep_pending, effects =
    Dsl.Runtime.handle_input after_word (key "u") context
  in
  ...
  let reset, effects =
    Dsl.Runtime.handle_input deep_pending (key "x") context
  in
  ...
  let after_word, effects =
    Dsl.Runtime.handle_input initial (key "w") context
  in
  expect
    (List.length effects = 1)
    "d w did not select its own shared-prefix rule";
  let after_word, _ = Dsl.Runtime.handle_input after_word (key "g") context in
  let after_word, _ = Dsl.Runtime.handle_input after_word (key "u") context in
  let complete, effects =
    Dsl.Runtime.handle_input after_word (key "w") context
  in
...
let test_adapter_configuration_isolation_history_and_replay () =
  ...
  expect
    (List.length (Runtime.change_ids step) = 1)
    "DSL text insertion did not create exactly one normal transaction";
  let left, _ =
    Runtime.execute_effects left ~input:(key "u") [ Model_effect.Undo ] |> must
  in
  expect_string ~expected:"alpha beta" ~actual:(contents left);
  let left, _ =
    Runtime.execute_effects left ~input:(key "r") [ Model_effect.Redo ] |> must
  in
  expect_string ~expected:"Xalpha beta" ~actual:(contents left);
  let replay_intent =
    ...
    | _ ->
        failf "DSL text transition did not produce one semantic replay intent"
  in
...
let tests =
  [
    ( "valid model and deterministic description",
      test_valid_model_and_description );
