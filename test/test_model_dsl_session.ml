...
let create ?trace ?(contents = "alpha beta") grammar =
  App.Session.create ~model:App.Session.Dsl ~dsl_model:grammar ~contents ?trace
...
let test_requires_a_compiled_grammar () =
  ...
  | Error (Error.Invalid_command_arguments message) ->
      expect
        (contains message "--model-dsl")
        "DSL startup reported an unhelpful missing-grammar error: %s" message
  | Error error ->
      failf "DSL startup returned the wrong error: %s" (Error.to_string error)
...
let test_status_bindings_and_text_edit () =
  ...
  expect
    (App.Session.contents session = "界alpha beta")
    "captured committed text did not traverse the normal runtime transaction \
     path";
...
let test_prefix_execution_and_mismatch () =
  ...
  expect
    (App.Session.contents session = "alpha beta")
    "a DSL prefix edited the document before completion";
  expect
    (Model_status.pending_input (App.Session.status session) = Some "d")
  ...
  expect
    (lines_contain pending_bindings "w")
  ...
  expect
    (Model_status.pending_input (App.Session.status session) = None)
...
let test_layout_save_rejects_dsl_without_writing () =
  ...
  let remove_if_present path = if Sys.file_exists path then Sys.remove path in
  Fun.protect
    ~finally:(fun () ->
    ...
    (fun () ->
      remove_if_present layout_path;
      (match
         App.File_io.save_atomic ~path:document_path ~contents:"alpha beta"
       with
      | Ok () -> ()
...
let tests =
  [
    ...
    ( "DSL session status, inspection, and text editing",
      test_status_bindings_and_text_edit );
    ( "DSL session prefix execution and mismatch",
      test_prefix_execution_and_mismatch );
    ( "DSL layout save rejects persistence without writing",
      test_layout_save_rejects_dsl_without_writing );
    ( "built-in session selection remains unchanged",
      test_builtin_model_creation_is_unchanged );
  ]
