...
let send_selection runtime input = Selection_runtime.handle_input runtime input |> must

let fold_vim runtime inputs =
  List.fold_left (fun runtime input -> fst (send_vim runtime input)) runtime inputs
...
let test_vim_pending_counts_and_cancellation () =
  ...
  expect (Model_status.metadata (Vim_runtime.status_after counted) = [ ("clipboard-slot", "unnamed"); ("count", "3") ])
    "count state was not exposed through status";
  let runtime, pending = send_vim runtime (key "d") in
  expect (Model_status.id (Vim_runtime.status_after pending) = "operator-pending")
  ...
  let runtime = fold_vim runtime [ key "2"; key "d"; named Input_event.Escape ] in
...
let test_vim_insert_change_and_character_edits () =
  ...
  let runtime =
    fold_vim runtime
      [ key "i"; committed "é界🙂"; named Input_event.Backspace; named Input_event.Enter; named Input_event.Escape ]
  in
  expect_string ~expected:"é界\n" ~actual:(text (Vim_runtime.history runtime));
  let runtime = vim "alpha beta" in
  let runtime = fold_vim runtime [ key "c"; key "w"; committed "X"; named Input_event.Escape ] in
  ...
  let runtime = fold_vim runtime [ key "a"; committed "界"; named Input_event.Escape ] in
...
let test_selection_first_semantics_and_multiple_selections () =
  ...
  expect (offsets (Selection_runtime.history selection_runtime) = [ (0, 6) ])
...
let test_cross_model_equivalence_and_replay_boundary () =
  ...
  in
  expect_string ~expected:(text (Vim_runtime.history vim_runtime))
    ~actual:(text (Selection_runtime.history selection_runtime));
  ...
  let replay =
    Replay.create ~document_id:"m3-replay" ~contents:"alpha beta"
      ~initial_selections:
        { Replay.selections = [ Selection_spec.make ~anchor_offset:0 ~head_offset:0 |> must ]; primary = 0 }
...
let tests =
  [
    ...
    ("Vim pending counts and cancellation", test_vim_pending_counts_and_cancellation);
