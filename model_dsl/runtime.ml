...
let stable compiled state_id =
  ...
  { grammar = compiled; stable_state = state_id; cursor = compiled_state.root; pending_input = [] }
...
let matching_edge node input =
  Compile.view_node node
  |> fun view ->
  List.find_opt (fun edge -> Input_event.binding_pattern_matches edge.pattern input) view.edges

let handle_input state input (_context : Editor_context.t) =
  match matching_edge state.cursor input with
  | None ->
      if state.pending_input = [] then (state, [])
      else (reset state, [])
  | Some edge -> (
      match (Compile.view_node edge.next).complete with
      | Some transition ->
          let next_state = stable state.grammar transition.target in
          let effects = List.map (effect_of_transition transition input) transition.effects in
          (next_state, effects)
      | None ->
          ( {
              state with
...
let status state =
  ...
  let pending_input =
    match state.pending_input with [] -> None | values -> Some (String.concat " " values)
  in
  Model_status.create ~id:compiled_state.ir.name ~label:compiled_state.ir.status_label
    ?pending_input ~input_mode:compiled_state.ir.input_mode ()
  |> Result.get_ok
...
let input_rules state =
  ...
  |> List.mapi (fun index edge ->
         ...
         in
         let next_status = Option.map (fun transition -> transition.Ir.target_name) transition in
         ...
         Input_rule.create
           ...
           ~pattern:(input_pattern edge) ~kind ~summary ?next_status
           ?selector_id ?transformation_id ()
         |> Result.get_ok)
