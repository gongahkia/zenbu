...
| `vim-ex-substitute-rejected` | rejected | Zenbu rejects native Vim `:substitute`: a leading `:` is not a Vim-model command prompt. Its generic palette replacement descriptors are separate host operations, not an Ex equivalent. [#27](https://github.com/gongahkia/zenbu/issues/27) records this decision; Ex grammar, Vim regexp semantics, ranges, flags, delimiters, replacement expansion, repeat/history, mappings, and compatibility claims remain absent. |
...
- `vim-delete-next-word` and `vim-ex-substitute-rejected`: Ex command language, Vim substitute ranges, flags, delimiters, replacement expansion, and repeat/history semantics, Vim regexp/search semantics, uppercase/global-register semantics, macro editing/persistence, broad motion/text-object coverage, linewise/global marks, full jump-source/per-window semantics, compatibility mappings, and terminal/GUI appearance parity.
