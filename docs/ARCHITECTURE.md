...
project them; neither folds nor the projection are document state. A host
embedding may likewise retain bounded version-bound decoration data, but no
provider callback crosses into the renderer or terminal. It handles host-only save,
...
completion, rename, accepted code-action and formatting edits, and accepted
server edits. See
