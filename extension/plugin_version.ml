    ...
    (Zenbu_kernel.Error.Extension_error
       {
         ...
         granted = [];
         message =
           "plugin version must use the supported SemVer core MAJOR.MINOR.PATCH form: "
           ^ value;
       })
...
let of_string value =
  ...
  | [ major; minor; patch ]
    when valid_component major && valid_component minor && valid_component patch ->
      (try Ok { major = int_of_string major; minor = int_of_string minor; patch = int_of_string patch }
       with Failure _ -> invalid value)
  | _ -> invalid value

let to_string value = Printf.sprintf "%d.%d.%d" value.major value.minor value.patch
