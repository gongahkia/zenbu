...
let demo_replay () =
  let initial_selections = { Replay.selections = [ selection 0 0 ]; primary = 0 } in
  match
    Replay.create ~document_id:"demo" ~contents:"world" ~initial_selections
      ~actions:
        [
          ...
          Replay.Intent
            (Intent.Set_selections { selections = [ selection 6 11 ]; primary = 0 });
          ...
          Replay.Transaction
            {
              ...
              edits = [ { Replay.start_offset = 11; stop_offset = 11; text = "!" } ];
...
let print_history history =
  List.iter
    (fun change ->
      ...
      let description = Option.value ~default:"-" (Transaction.description metadata) in
      Printf.printf "change %d: %s %s (%s), v%d -> v%d\n" (History.change_id change)
        (Transaction.source_to_string (Transaction.source metadata)) intent description
...
let print_result history =
  ...
  List.iter
    (fun selection ->
      Printf.printf " %d:%d" (Anchor.byte_offset (Selection.anchor selection))
...
let () =
  let replay =
    ...
    | [ _; "demo" ] -> demo_replay ()
    | [ _; "replay"; path ] -> (
        match Replay.of_string (read_file path) with Ok replay -> replay | Error error -> fail error)
    | _ -> usage ()
  in
  match Replay.run replay with Ok history -> print_result history | Error error -> fail error
