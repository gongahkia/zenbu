...
let enabled ~capacity =
  if capacity <= 0 then
    Error
      (Zenbu_kernel.Error.Invalid_provenance "trace capacity must be positive")
...
let emit_lazy trace create =
  ...
  | Disabled -> ()
  | Enabled value ->
      if Queue.length value.events = value.capacity then
...
let clear = function
  | Disabled -> ()
  | Enabled value -> Queue.clear value.events
