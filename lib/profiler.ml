...
type aggregate = { key : key; count : int; total_seconds : float; max_seconds : float }
...
let enabled ~capacity =
  if capacity <= 0 then
    Error (Zenbu_kernel.Error.Invalid_provenance "profile capacity must be positive")
  ...
  | Enabled value ->
      ...
      Fun.protect
        ~finally:(fun () ->
          ...
          if Queue.length value.samples = value.capacity then ignore (Queue.take value.samples);
          Queue.add { key = { stage; model_id }; duration } value.samples)
        f

let reset = function Disabled -> () | Enabled value -> Queue.clear value.samples
let same_key left right = left.stage = right.stage && left.model_id = right.model_id
  ...
  | Enabled value ->
      let add (aggregates : aggregate list) (sample : sample) =
        let rec loop before = function
          | [] -> List.rev ({ key = sample.key; count = 1; total_seconds = sample.duration; max_seconds = sample.duration } :: before)
          | aggregate :: rest when same_key aggregate.key sample.key ->
              List.rev_append before
                ({ aggregate with count = aggregate.count + 1; total_seconds = aggregate.total_seconds +. sample.duration; max_seconds = max aggregate.max_seconds sample.duration } :: rest)
      ...
      |> List.sort (fun left right ->
             match String.compare (stage_name left.key.stage) (stage_name right.key.stage) with
             | 0 -> Option.compare String.compare left.key.model_id right.key.model_id
             | value -> value)
...
let aggregate_mean_seconds value = value.total_seconds /. float_of_int value.count
