...
let source_of_string = function
  ...
  | "replay" -> Ok Replay
  | "system" -> Ok System
  | value -> Error (Error.Malformed_replay ("unknown transaction source " ^ value))
              ...
              if edits_conflict edit other then
                Error
                  (Error.Overlapping_edits
                     { first_index = edit.ordinal; second_index = other.ordinal })
              else inner tail
        in
...
let create ~document_id ~source_version ~edits ?selection_change ~metadata () =
  if
    List.exists
      ...
      edits
  then Error (Error.Invalid_range "an edit does not belong to the transaction source snapshot")
  else
    match selection_change with
    | Some selections
      when not (Document_id.equal document_id (Selection_set.document_id selections)) ->
        Error (Error.Invalid_selection_set "selection change names another document")
    | Some selections
      when not (Document_version.equal source_version (Selection_set.version selections)) ->
        Error (Error.Invalid_selection_set "selection change names another document version")
    | _ when edits = [] && Option.is_none selection_change -> Error Error.Empty_transaction
    | _ ->
        let indexed = List.mapi (fun ordinal edit -> { edit; ordinal }) edits in
        match validate_conflicts indexed with
        | Error _ as error -> error
        | Ok () -> Ok { document_id; source_version; edits = List.sort compare_indexed indexed; selection_change; metadata }
