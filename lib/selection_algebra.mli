(** Model-neutral operations over an immutable editor selection set.
    ...
    bypass transaction validation. Regex operations use the OCaml [Str]
    dialect and reject zero-width matches and byte offsets that would split a
    UTF-8 code point. *)

type direction = Forward | Backward

val select_regex :
  Editor_context.t -> pattern:string -> (Model_intent.t, Zenbu_kernel.Error.t) result
