...
let snapshot value =
  match
    Document_snapshot.make ~document_id:value.id ~version:value.version
      ~buffer:value.buffer ~selections:value.selections
  with
  | Ok snapshot -> snapshot
  | Error error ->
      failwith ("document invariant violated: " ^ Error.to_string error)
...
let selection_set_of_specs ~id ~version ~buffer ~specs ~primary =
  let rec make_selections = function
    ...
    | spec :: rest -> (
        match
          Anchor.make ~document_id:id ~version
  ...
  | Ok selections -> (
      ...
      | Ok selection_set -> (
          let provisional =
            { id; version; buffer; selections = selection_set }
          in
          match
            Document_snapshot.validate_selection_set (snapshot provisional)
              selection_set
          with
          | Error _ as error -> error
          | Ok () -> Ok selection_set))
  ...
  | Ok buffer -> (
      let initial_selections =
        if initial_selections = [] then
          ...
          | Error error ->
              failwith ("impossible default selection: " ^ Error.to_string error)
      ...
      | Error _ as error -> error
      | Ok selections ->
          Ok { id; version = Document_version.initial; buffer; selections })
...
let validate_transaction value transaction =
  if not (Document_id.equal value.id (Transaction.document_id transaction)) then
    ...
    not
      (Document_version.equal value.version
         (Transaction.source_version transaction))
  then
    Error
      (Error.Stale_version
         {
           ...
           actual =
             Document_version.to_int (Transaction.source_version transaction);
         })
  else
    ...
    let rec validate_edits = function
      ...
      | edit :: rest -> (
          match
            Document_snapshot.validate_range source_snapshot (Edit.range edit)
          with
          | Error _ as error -> error
    ...
    | Ok () -> (
        ...
        | Some selections -> (
            match
              Document_snapshot.validate_selection_set source_snapshot
                selections
            with
            | Error _ as error -> error
...
let transform_offset edits offset =
  let delta_before point =
    List.fold_left
      (fun delta edit ->
        ...
        else if start < point && start = stop then
          delta + String.length (Edit.text edit)
        else delta)
      0 edits
  in
  let insertions_at point =
    List.fold_left
      (fun length edit ->
        if start_offset edit = point && Edit.is_insertion edit then
          length + String.length (Edit.text edit)
        else length)
      0 edits
  in
  let active_range =
  ...
  | Some edit ->
      let start = start_offset edit in
      start + delta_before start + insertions_at start
      + String.length (Edit.text edit)
...
let rebase_selections ~version ~edits selections =
  Selection_set.map_anchors selections ~f:(fun anchor ->
      Anchor.rebase anchor ~version
        ~byte_offset:(transform_offset edits (Anchor.byte_offset anchor)))
...
let apply ?result_version value transaction =
  ...
  | Ok _ -> (
      let target_version =
        match result_version with
        | Some version -> version
        | None -> Document_version.successor value.version
      in
      if Document_version.compare target_version value.version <= 0 then
        ...
        | Ok buffer -> (
            ...
            in
            match
              rebase_selections ~version:target_version ~edits source_selections
            with
            | Error _ as error -> error
            | Ok selections ->
                Ok
