...
let range_start selection =
  Anchor.byte_offset (Range.start (Selection.range selection))

let range_stop selection =
  Anchor.byte_offset (Range.stop (Selection.range selection))
...
let create ~primary selections =
  ...
  if count = 0 then
    Error (Error.Invalid_selection_set "a selection set cannot be empty")
  else if primary < 0 || primary >= count then
    Error
      (Error.Invalid_selection_set "primary selection index is out of bounds")
  else
    ...
    if
      List.exists
        (fun selection ->
          (not
             (Document_id.equal document_id (Selection.document_id selection)))
          || not (Document_version.equal version (Selection.version selection)))
        selections
    then
      Error (Error.Invalid_selection_set "all selections must share a snapshot")
    else
      ...
      | Ok () -> (
          let rec find index = function
            ...
            | selection :: _ when Selection.equal selection primary_selection ->
                Ok index
          ...
          | Error _ as error -> error
          | Ok primary_index ->
              Ok { selections = sorted; primary_index; document_id; version })
...
let map_anchors value ~f =
  let mapped =
    List.map
      (fun selection -> Selection.map_anchors selection ~f)
      value.selections
  in
  ...
  | Ok selections -> (
      ...
      let rec deduplicate previous = function
        ...
        | selection :: rest ->
            if
              Option.fold ~none:false ~some:(Selection.equal selection) previous
            then deduplicate previous rest
      ...
      let rec index_of index = function
        | [] ->
            Error
              (Error.Invalid_selection_set "mapped primary selection was lost")
        | selection :: _ when Selection.equal selection mapped_primary ->
            Ok index
